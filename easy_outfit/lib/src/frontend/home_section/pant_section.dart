...
import 'package:easy_outfit/src/frontend/home_section/shoe_section.dart';
import 'package:easy_outfit/src/backend/models/cloth_model.dart';
import 'package:flutter/material.dart';
import 'package:google_fonts/google_fonts.dart';

class PantSection extends StatefulWidget {
  String user;
  final int idShirt;
  final int weekday;
  final int weekyear;
  PantSection({super.key, required this.user, required this.idShirt, required this.weekday, required this.weekyear});

  @override
  _PantSectionState createState() => _PantSectionState(user: user, idShirt: idShirt, weekday: weekday, weekyear: weekyear);
}

class _PantSectionState extends State<PantSection>
    with SingleTickerProviderStateMixin {
  final String user;
  final int idShirt;
    final int weekday;
  final int weekyear;
  _PantSectionState({required this.user, required this.idShirt, required this.weekday, required this.weekyear});
                      ...
                      return GestureDetector(
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => ShoeSection(user: _clothController.username, idShirt: idShirt, idPants: snapshot.data![index].id ?? -1, weekday: weekday, weekyear: weekyear,),
                            ),
                          );
                        },
                        child: Card(
